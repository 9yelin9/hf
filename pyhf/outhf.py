...
class OutHF:
	...
	def ShowBand(self, N, U, ax, Nk, is_unfold):
		...
		if is_unfold:
			#axins = ax.inset_axes([0.92, 0.32, 0.052, 0.37])
	...
	def ShowBandDOS(self, N, U, Nk=0, ep=0.1, is_unfold=0):
		...
		fig, ax = plt.subplots(1, 2, width_ratios=[3.5, 1], figsize=(7, 5), constrained_layout=True)
	...
	def ShowPhase(self, Nlevel=10, specific_init=0, xmin=0, xmax=0, ymin=0, ymax=0):
		...
		cf = ax.contourf(X, Y, Z, levels=np.linspace(0.1, self.Nb//2, Nlevel), cmap='plasma')
		cb = plt.colorbar(cf, format='%.1f')
		cb.set_ticks([0.1, self.Nb//2])
		cb.set_ticklabels(['0.1', '3.0'])
