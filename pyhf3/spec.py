...
class Spec:
	...
	def GenEnergy_(self):
		...
		t1 = time.time()
		print('GenEnergy_(%s) : %fs' % (en, t1-t0))
	...
	def GenFileList_(self, dtype):
		...
		fn_list = ['%s/%s/%s' % (self.path_output, d, f)\
				for d in os.listdir(self.path_output)   if re.search('J', d)\
		...
		t1 = time.time()
		print('GenFileList_(%s, %s, %s) : %fs' % (pn, bn, un, t1-t0))
